...
main($_POST['ref'],$_POST['idOp']);

/**
 * Main method for testing purposes
 */
function main($orderID,$idOp){
	...
	if( empty ($_GET)){
		initialOperationV1($orderID,$idOp);
	}
	else{
		if(!empty ($_POST)){
 ...
 */
function initialOperationV1($orderID,$idOp) {
		
	$protocolVersion = "";
	$cardDataInfoRequest = new RESTInitialRequestMessage();
	
	// Operation mandatory data
	$cardDataInfoRequest->setAmount("3000"); // i.e. 1,23 (decimal point depends on currency code)
	...
	$cardDataInfoRequest->useReference($idOp);
	...
	//Method that gives the request Result (OK/KO/AUT)
