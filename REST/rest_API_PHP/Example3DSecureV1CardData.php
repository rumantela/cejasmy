	...
	if( empty ($_GET)){
		...
		if(!empty ($_POST)){
			...
			if ($_GET['urlOK'] == 'yes'){
				...
				echo "OPERATION: </br>";
				var_dump($operation);
				echo "Llamando a la función Challenge Request...</br>";
...
function initialOperationV1($orderID) {
	...
	$cardDataInfoRequest->setTerminal("1");
    ...
    $cardDataInfoRequest->setCardExpiryDate("4912");
...
function authenticationOperationV1($cardDataInfoRequest, $protocolVersion) {
    ...
    $operationRequest->setCardExpiryDate("4912");
	...
	if ($response->getResult() == RESTConstants::$RESP_LITERAL_AUT) {
		...
		$termUrl = "http://localhost/cejasmy/REST/rest_API_PHP/Example3DSecureV1Reference.php?urlOK=yes&ope=" . $ope;
...
function challengeRequestV1($operation) {
    ...
    //Card Data information
    $challengeRequest->setCardNumber("4548810000000003");
